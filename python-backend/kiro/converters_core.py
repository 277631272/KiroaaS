...
def sanitize_json_schema(schema: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for key, value in schema.items():
        ...
        if key == "properties" and isinstance(value, dict):
            result[key] = {
                prop_name: (
                    sanitize_json_schema(prop_value)
...
def ensure_first_message_is_user(
    messages: List[UnifiedMessage],
) -> List[UnifiedMessage]:
    """
...
def normalize_message_roles(messages: List[UnifiedMessage]) -> List[UnifiedMessage]:
    ...
    if converted_count > 0:
        logger.debug(
            f"Normalized {converted_count} message(s) with unknown roles to 'user'"
        )
...
def ensure_alternating_roles(messages: List[UnifiedMessage]) -> List[UnifiedMessage]:
    ...
    if synthetic_count > 0:
        logger.debug(
            f"Inserted {synthetic_count} synthetic assistant message(s) to ensure alternation"
        )
...
def build_kiro_payload(
    ...
    if truncation_system_addition:
        full_system_prompt = (
            full_system_prompt + truncation_system_addition
            if full_system_prompt
